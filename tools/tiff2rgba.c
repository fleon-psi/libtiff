...

{
    ...
    if (no_alpha)
    {
        ...
        while (count > 0)
        {
            ...
            *(dst++) = (*(src) * *(src+3) / 0xFF) + (background * 0xFF * (0xFF - *(src+3)) / 0xFE01); src++;
            *(dst++) = (*(src) * *(src+2) / 0xFF) + (background * 0xFF * (0xFF - *(src+2)) / 0xFE01); src++;
            *(dst++) = (*(src) * *(src+1) / 0xFF) + (background * 0xFF * (0xFF - *(src+1)) / 0xFE01); src++;
            src++;
            count--;
        }
    }
